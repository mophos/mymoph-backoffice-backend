...
export class TaxModel {
  ...
  async softDeleteYearAndDocuments(id: number, actorId?: string) {
    await this.db.transaction(async (trx) => {
      await trx(TAX_DOCUMENTS_TABLE)
        .where({ tax_year_id: id })
        .andWhere('is_active', 1)
        .update({
          is_active: 0,
          updated_by: actorId ?? null,
  ...
  async softDeleteDocument(id: string, actorId?: string) {
    await this.db(TAX_DOCUMENTS_TABLE)
  ...
  async listDocumentPathsByYearId(yearId: number) {
    return this.db(TAX_DOCUMENTS_TABLE)
      .where({ tax_year_id: yearId, is_active: 1 })
